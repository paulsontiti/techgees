      ...
      <div className="flex flex-col pt-2">
       ...
       {Array.isArray(categories) &&
            categories.map((category) => {
              return (
                <span key={category.id} className="text-xs mr-2 italic bg-sky-100 rounded-full mt-4 py-1 px-2">
                  {category.name}
                </span>
              );
            })}
      ...
      <div>
          ...
          <div>
            ...
            {childrenCourses.map((course) => {
              return (
                <span key={course.id} className="text-xs mr-2 italic p-2 bg-sky-100 rounded-full mt-4 py-1 px-2">
                  {course.title}
                </span>
              );
            })}</div>
          ...
          preRequisiteCourses.length > 0 && (
            <div>
              ...
              {preRequisiteCourses.map((course) => {
                return (
                  <span key={course.id} className="text-xs mr-2 italic bg-sky-100 rounded-full mt-4 py-1 px-2">
                    {course.title}
                  </span>
                );
              })}
