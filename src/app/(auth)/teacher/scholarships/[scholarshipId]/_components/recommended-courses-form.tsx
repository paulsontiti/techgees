...
function RecommendedCoursesForm(
  {recommendedCourses,courses,courseId}:
  {recommendedCourses:Course[],courses:Course[],courseId:string}) {
const [editing,setEditing] = useState(false)
const router = useRouter()

const [open, setOpen] = React.useState(false)
const [loading, setLoading] = React.useState(false)
const [selectedCourses,setSelectedCourses] = useState<Course[]>(recommendedCourses)
    ...
    const onSubmit = async(selectedCourseIds:string[])=>{
        try{
            await axios.post(`/api/courses/${courseId}/recommended-courses`,selectedCourseIds)
    ...
    <div>
       ...
       :
         <div className={cn('text-sm mt-2',
            recommendedCourses.length === 0 && "text-slate-500 italic"
         )}>{recommendedCourses.length > 0 ? <div className="flex gap-2 flex-wrap">
...
export default RecommendedCoursesForm