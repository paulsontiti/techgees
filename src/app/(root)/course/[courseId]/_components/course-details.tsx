...
import { bgNeutralColor, bgNeutralColor2, textPrimaryColor } from '@/utils/colors';
...
const [course,setCourse] = useState<CourseType | null | undefined>(undefined);
const [startedCourse,setStartedCourse] = useState<boolean | undefined>(undefined);
  ...
  return (
    ...
    <div className="border-black border-2 my-16 w-full md:w-[400px] py-8 px-2 rounded-xl 
      ...
      <div className="flex flex-col items-center justify-center gap-y-4 ">
        ...
        {startedCourse !== undefined ? <Button variant={`${startedCourse ? "success" : "destructive"}`} size="sm">
        {`${startedCourse ? "Started" : "Not started"}`}</Button> : <Skeleton className='w-32 h-10 bg-red-300'/>}
      </div>
      <Separator />
...
<Skeleton className={`w-full mt-4 h-[200px] ${bgNeutralColor2}`}/>
   ...
   {courseChildren?.length === 0 &&    <CourseChaptersAndSessionsDetails courseId={courseId}/>}
