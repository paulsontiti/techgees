...
function SessionComments({
  ...
  useEffect(()=>{
    (
      async()=>{
        try{
          const likesRes = await axios.get(`/api/sessions/${sessionId}/likes-count`);
          setNumberOfLikes(likesRes.data);

          const disLikesRes = await axios.get(`/api/sessions/${sessionId}/dislikes-count`);
          setNumberOfDisLikes(disLikesRes.data);

          const studentsRes = await axios.get(`/api/sessions/${sessionId}/started-students-count`);
          setNumberOfStudents(studentsRes.data);

          const ratingsRes = await axios.get(`/api/sessions/${sessionId}/average-rating`);
          setRating(ratingsRes.data);

          const numberofRatingsRes = await axios.get(`/api/sessions/${sessionId}/rating-count`);
          setNumberOfRatings(numberofRatingsRes.data);

          const hasLikedRes = await axios.get(`/api/sessions/${sessionId}/has-liked`);
          setHasLiked(hasLikedRes.data);

          const hasDisLikedRes = await axios.get(`/api/sessions/${sessionId}/has-disliked`);
          setHasDisLiked(hasDisLikedRes.data);

          const hasRatedRes = await axios.get(`/api/sessions/${sessionId}/has-rated`);
